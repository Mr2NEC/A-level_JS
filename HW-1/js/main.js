...
function postMessage () {
    jsonPost("http://students.a-level.com.ua:10012", {func: 'addMessage', nick: input1Elem.value, message: input2Elem.value})
    .then(()=> { while (divElem.firstChild) {
        divElem.removeChild(divElem.firstChild);
      }}).then(()=> getMessage())
}

function getMessage(){
 jsonPost("http://students.a-level.com.ua:10012", {func: 'getMessages', }).then(data => {
    for(let j in data.data){ cteateMessages(data.data[j]);} 
    nextMessageId = data.nextMessageId;})
}
getMessage()
function cteateMessages(objElem){
    let divMessage = document.createElement("div");
    divMessage.style = "border: 1px solid black; padding: 10px; background: #00FFFF; display: flex; align-items: center; flex-direction: column;";
    divMessage.innerHTML = `<div style = 'color: red;'>Nick: ${objElem.nick}</div><div style = 'color: green;'>Message: ${objElem.message}</div><div style = 'color: black;'>Timestamp: ${timeConverter(objElem.timestamp)}</div>`;
